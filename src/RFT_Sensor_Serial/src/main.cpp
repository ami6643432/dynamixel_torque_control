...
#include "rft_sensor_serial/rft_operation.h"	// this file will be automatically generated during build operation.
...
bool rft_operation_service(rft_sensor_serial::rft_operation::Request &req, rft_sensor_serial::rft_operation::Response &res);
uint8_t rft_response_wait(uint8_t opType);
uint8_t rft_response_display(uint8_t opType);
uint8_t rft_send_command(rft_sensor_serial::rft_operation::Request &req);
...
{
	ros::init(argc, argv, "rft_sensor_serial");
	ros::NodeHandle nh;
	...
	//service
	ros::ServiceServer op_service = nh.advertiseService("rft_serial_op_service", rft_operation_service);
	//publisher
	...
	while(ros::ok())
	{
		...
		if( (RFT_SENSOR.m_nCurrMode == CMD_FT_CONT) && isSensorOk )
		{
			...
			ft_data.wrench.force.x = RFT_SENSOR.m_RFT_IF_PACKET.m_rcvdForce[0];
			ft_data.wrench.force.y = RFT_SENSOR.m_RFT_IF_PACKET.m_rcvdForce[1];
			ft_data.wrench.force.z = RFT_SENSOR.m_RFT_IF_PACKET.m_rcvdForce[2];
			ft_data.wrench.torque.x = RFT_SENSOR.m_RFT_IF_PACKET.m_rcvdForce[3];
			ft_data.wrench.torque.y = RFT_SENSOR.m_RFT_IF_PACKET.m_rcvdForce[4];
			ft_data.wrench.torque.z = RFT_SENSOR.m_RFT_IF_PACKET.m_rcvdForce[5];
...
void init_param(ros::NodeHandle *n)
{
	...
	if(!n->getParam("RFT_COM_PORT", g_com_port))
	{
		ROS_WARN("RFT Serial port is not defined, The port is initialized with %s", g_com_port.c_str());
		n->setParam("RFT_COM_PORT", g_com_port.c_str()); // set param
		n->getParam("RFT_COM_PORT", g_com_port);    // checking 
	}
	ROS_INFO("RFT Serial port: %s", g_com_port.c_str());
	
	// parameter serial port device
	if(!n->getParam("RFT_COM_BAUD", g_baud_rate))
	{
		ROS_WARN("The baud-rate of RFT Serial port is not defined, The baud-rate is initialized with %d", g_baud_rate);
		n->setParam("RFT_COM_BAUD", g_baud_rate); // set param
		n->getParam("RFT_COM_BAUD", g_baud_rate);    // checking 
	}
	...
	n->setParam("RFT_SERIAL_SW_VER", ROS_RFT_SERIAL_SW_VER);
}
...
uint8_t rft_send_command(rft_sensor_serial::rft_operation::Request &req)
{
	...
	switch( req.opType )
	{
		...
		case CMD_SET_COMM_BAUDRATE:			// Only UART version, CAN : 1Mbps Fixed
			...
			RFT_SENSOR.set_Comm_Speed(req.param1);		
		break;
		...
		case CMD_SET_FT_FILTER:	
			...
			RFT_SENSOR.set_FT_Filter_Type(req.param1, req.param2);			
		break;
		...
		case CMD_SET_CONT_OUT_FRQ: 	
			...
			RFT_SENSOR.set_FT_Cont_Interval(req.param1);		
		break;
		...
		case CMD_SET_BIAS:		
			...
			RFT_SENSOR.set_FT_Bias(req.param1);			
		break;
...
uint8_t rft_response_display(uint8_t opType)
{
	...
	{
	case CMD_GET_PRODUCT_NAME:
		ROS_INFO("%s", RFT_SENSOR.m_RFT_IF_PACKET.m_rcvd_product_name);
		break;
	case CMD_GET_SERIAL_NUMBER:
		ROS_INFO("%s", RFT_SENSOR.m_RFT_IF_PACKET.m_rcvd_serial_number);
		break;
	case CMD_GET_FIRMWARE_VER:
		ROS_INFO("%s", RFT_SENSOR.m_RFT_IF_PACKET.m_rcvd_firmware_version);
		break;
	case CMD_SET_ID:
		result = NOT_SUPPORTED_CMD;
		break;
	...
	case CMD_SET_COMM_BAUDRATE:
		ROS_INFO("Cmd Type: %d, Result: %d, Err. Code: %d", RFT_SENSOR.m_RFT_IF_PACKET.m_response_cmd, RFT_SENSOR.m_RFT_IF_PACKET.m_response_result, RFT_SENSOR.m_RFT_IF_PACKET.m_response_errcode );
		result = RFT_SENSOR.m_RFT_IF_PACKET.m_response_errcode;
		break;
	case CMD_GET_COMM_BAUDRATE:
		ROS_INFO("Baud: %d, new Baud: %d", RFT_SENSOR.m_RFT_IF_PACKET.m_rcvd_curr_comm_baudrate, RFT_SENSOR.m_RFT_IF_PACKET.m_rcvd_set_comm_baudrate);
		break;
	case CMD_SET_FT_FILTER:
		ROS_INFO("Cmd Type: %d, Result: %d, Err. Code: %d", RFT_SENSOR.m_RFT_IF_PACKET.m_response_cmd, RFT_SENSOR.m_RFT_IF_PACKET.m_response_result, RFT_SENSOR.m_RFT_IF_PACKET.m_response_errcode );
		result = RFT_SENSOR.m_RFT_IF_PACKET.m_response_errcode;
		break;
	case CMD_GET_FT_FILTER:
		ROS_INFO("filter type: %d, sub. setting: %d", RFT_SENSOR.m_RFT_IF_PACKET.m_rcvd_filter_type, RFT_SENSOR.m_RFT_IF_PACKET.m_rcvd_filter_setting_value);
		break;
	case CMD_FT_ONCE:
	...
	case CMD_SET_CONT_OUT_FRQ:
		...
		break;
	case CMD_GET_CONT_OUT_FRQ:
		ROS_INFO("output frq: %d", RFT_SENSOR.m_RFT_IF_PACKET.m_rcvd_tx_frq);
		break;
	case CMD_SET_BIAS:
		// THERE IS NO - RESPONSE PACKET
		break;
