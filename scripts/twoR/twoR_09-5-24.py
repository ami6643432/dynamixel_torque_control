...
def main():
    ...
    try:
        ...
        while True:
            ...
            tau_Gravity = kg * ( (m1+m2) * l1 * 9.81 * np.sin(pos*np.pi/180) + m2 * l2 * 9.81 * np.sin((pos+pos2)*np.pi/180) )
            ...
            # tau_Gravity2 = kg2 * 9.81 * 0.5 * np.cos(pos2*np.pi/180)

            tau_Gravity2 = kg * m2 * l2 * 9.81 * np.sin((pos + pos2)*np.pi/180)

            # tau2 = (kp2 * ep2 + kd2 * ev2)
            tau2 = kp2 * ep2 + kd2 * ev2+ tau_Gravity2
            # tau2 = tau_Gravity2
            ...
            #print gravity torque
            print("Tau_Gravity2 : ", tau_Gravity2)
            print("Tau2 : ", tau2)


            dynamixel_obj1.set_goal_curr_callback(int(round(tau1)))
            dynamixel_obj2.set_goal_curr_callback(int(round(tau2)))
